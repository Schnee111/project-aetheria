      ...
      <AnimatePresence mode="wait">
        {isNarrator ? (
          ...
          <motion.div
            ...
            className="absolute inset-0 flex items-end justify-center pb-[10vh] lg:pb-[12vh] cursor-pointer bg-gradient-to-t from-black/50 via-black/10 to-transparent"
            onClick={handleClick}
          >
            <motion.div
          ...
          <motion.div key="dialog" className="absolute inset-0 cursor-pointer" onClick={handleClick}>
            ...
            >
              <div className="relative w-full bg-black/30 backdrop-blur-md border border-white/5 rounded-2xl p-4 pt-3 shadow-[0_8px_32px_rgba(0,0,0,0.5)] lg:min-h-[140px] lg:p-8 lg:pt-6">
                <AnimatePresence>
                  {speakerName && (
                    <motion.div
                      ...
                      key={speakerName}
                      className="inline-block mb-1.5 lg:mb-4 relative z-20"
                    >
                      <span className="text-[11px] lg:text-xs font-semibold tracking-widest text-gray-300 uppercase border-b border-white/20 pb-1">
                        {speakerName}
                      </span>
                    </motion.div>
                  )}
                ...
                <div className="relative z-10">
                  <p className="text-base leading-relaxed font-body font-medium text-gray-100 min-h-[64px] drop-shadow-md lg:min-h-[60px] lg:text-xl lg:leading-relaxed">
                    {displayedText}
                    {!isComplete && (
                ...
                <AnimatePresence>
                  {isComplete && !line.unskippable && !line.autoAdvance && (
                    <motion.div
                      className="absolute bottom-3 right-5 flex items-center gap-1.5 text-[10px] font-semibold text-gray-400 tracking-[0.2em] uppercase z-20 lg:bottom-6 lg:right-8 lg:text-xs"
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                    >
